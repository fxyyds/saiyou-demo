  ...
  <div class="mx-4">
    ...
    <van-cell-group class="border mt-4" inset>
      ...
      <van-field input-align="right"
        ...
        <template #input>
          <van-radio-group v-model="checked" @change="typeChange" direction="horizontal">
            <van-radio name="1">单选题</van-radio>
            <br/>
            <van-radio name="2">多选题</van-radio>
            <br/>
            <van-radio name="3">判断题</van-radio>
            <br/>
<!--            <van-radio name="4">主观题</van-radio>-->
          </van-radio-group>
        </template>
      </van-field>
      ...
      <div class="p-2">
        <van-button block plain @click="add">添加题目</van-button>
      </div>
    </van-cell-group>
  </div>
</template>
...
const add = async () => {
  ...
  data.question.type = checked.value;
  const res = await QuestionControllerService.addQuestionUsingPost({
    content: data.question.content,
    questionAnswerList: data.question.questionAnswerList,
    questionBankId: data.question.questionBankId,
    type: Number(data.question.type)
  });
...
onMounted(() => {
  if (route.query.questionBankId) {
    data.question.questionBankId = route.query.questionBankId;
  }
})
