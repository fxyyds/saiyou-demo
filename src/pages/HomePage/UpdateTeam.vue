<template>
  <div class="mt-2 mx-4">
    <div>
      ...
      />
      <div class="mt-4">
        <van-button plain block hairline type="primary" @click="onSumbit">编辑队伍</van-button>
      </div>
    </div>
  </div>
...
const onSumbit = async () => {
  ...
  const res = await TeamControllerService.updateTeamUsingPost({...data.team, teamStatus: checked.value})
...
onMounted(async () => {
  ...
  if (res.data && res.code === 0) {
    data.team = res.data
    data.team.tags=res.data.tagList
    data.team.wantToTags=res.data.wantToTagList

    if (res.data.teamAvatarUrl){
      data.fileList = [{url: res.data.teamAvatarUrl}]
    }
  }
})
