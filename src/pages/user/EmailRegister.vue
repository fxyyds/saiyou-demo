      ...
      <div class="relative max-w-md mx-auto">
        <div class="form relative z-20 p-8 bg-white shadow-lg">
          ...
          <div class="flex flex-col gap-2 mt-7 mb-5">
            <div>
              ...
              <div class="ipt-wrap relative mt-1 rounded overflow-hidden">
                <input
                    v-model="data.emailRegister.userName"
            ...
            <div>
              ...
              <div class="ipt-wrap relative mt-1 rounded overflow-hidden">
                <input
                    v-model="data.emailRegister.invitationCode"
                    class="ipt w-full px-3 py-3"
                    type="text"
                    id="invitationCode"
                    placeholder="请输入邀请码"
          ...
          <div class="flex justify-center items-center mt-7 text-center">
            ...
            <span class="mx-4">或者使用其他方式注册</span>
            <span class="h-px flex-1 bg-gray-200"></span>
          </div>
...
onMounted(() => {
  if (route.query.code) {
    data.emailRegister.invitationCode = route.query.code
  }
})

onMounted(() => {
  const storedCountdown = localStorage.getItem("emailCountdown");
  if (storedCountdown && parseInt(storedCountdown) > 0) {
    ...
    const timer = setInterval(() => {
      ...
      if (data.countdown === 0) {
        ...
        // 更新本地存储中的倒计时值
        localStorage.setItem("emailCountdown", data.countdown.toString());
      }
    }, 1000);
  }
})
...
const login = async () => {
  const res = await UserControllerService.userEmailRegisterUsingPost(data.emailRegister);
  if (res.data && res.code === 0) {
    showSuccessToast("注册成功")
    await router.push("/email/login")
  }
}
