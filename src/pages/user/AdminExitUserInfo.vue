    ...
    <van-cell-group>
      ...
      <van-cell>
        <div class="items-center justify-between flex">
          ...
          <div class="flex items-center justify-end">
            <div>{{ data.user.email ? data.user.email : '暂未绑定' }}</div>
            <div>
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                   stroke="currentColor" class="w-5 h-5">
                <path stroke-linecap="round" stroke-linejoin="round" d="m8.25 4.5 7.5 7.5-7.5 7.5"/>
      ...
      <van-cell @click="userProfileShow=true">
        <div class="items-center justify-between flex">
          ...
          <div class="flex items-center justify-end">
            <div>
              {{
                data.user.userProfile ? data.user.userProfile && data.user.userProfile.length < 12 ? data.user.userProfile : data.user.userProfile.slice(0, 12) + '...' : '还未设置个性签名哦~'
              }}
            </div>
            <div>
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
...
onMounted(async () => {
  ...
  const res = await UserControllerService.getUserByIdUsingGet(route.params.id)
  if (res.data && res.code === 0) {
    data.user = res.data
    data.fileList = [{url: res.data.userAvatar ? res.data.userAvatar : avatar}]
  }
})
...
const userBindEmail = async () => {
  ...
  if (res.data && res.code === 0) {
    ...
    data.user.email = data.emailRegister.emailAccount
  }
}
...
const afterRead = async (file) => {
  ...
  if (res.data && res.code === 0) {
    ...
    data.fileList = [{url: data.user.userAvatar ? data.user.userAvatar : avatar}]
  }
};
