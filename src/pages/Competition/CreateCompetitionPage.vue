<template>
  <div class="mt-2 mx-4">
    <div>
      ...
      <div class="mt-4">
        <van-button block hairline plain type="primary" @click="onSumbit">创建赛事</van-button>
      </div>
        ...
        <div class="mt-2">
          ...
          <div v-else class="h-44 mt-6 col-span-3">
            <n-empty description="没找到考试哦~">
              <template #extra>
                <!--                todo-->
                <n-button @click="router.push('/exam/add')" size="small">
                  去创建
                </n-button>
              </template>
            </n-empty>
...
const onSumbit = async () => {
  ...
  const res = await CompetitionControllerService.addCompetitionUsingPost(data.competition)
