...
<!--                <div v-else-if="question.type === 4" class="text-sm text-gray-500">主观题</div>-->
          ...
          <!--            </template>-->
          <!--          </van-cell>-->
          <van-empty v-if="data.back.questionList.length <= 0" description="暂无数据"/>
        </van-checkbox-group>
      </div>
    </van-action-sheet>
  </div>
...
const add = async () => {
  const res = await ExamPaperControllerService.updateExamPaperUsingPost({
    id: data.paper.id,
...
onMounted(async () => {
  ...
  const res = await ExamPaperControllerService.listExamPaperAndQuestionVoByPageUsingPost({
    id: id,
  });
  if (res.code === 0 && res.data) {
    data.paper = res.data;
...
watchEffect(() => {
  data.paper.totalScore = data.paper.questionList.reduce((prev, cur) => {
